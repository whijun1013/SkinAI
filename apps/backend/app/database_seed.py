...
def seed_cosmetics_data(db: Session):
    ...
    json_path = os.path.join(
        ...
        "data", "cosmetic_master_seed.json"
    )

    if not os.path.exists(json_path):
        print(f"\n[WARNING] Source JSON database not found at: {json_path}")
        print("[WARNING] The primary cosmetic seed data will be skipped.")
        print("[WARNING] To fix this, run the data crawler (data_tools/primary_retailer_crawler.py)")
        print("[WARNING] or place the local seed 'cosmetic_master_seed.json' in apps/backend/data/\n")
        return
    ...
    try:
        ...
        for item in data:
            ...
            if (brand, product_name) in existing_products:
                continue
...
def seed_medications_data(db: Session):
    ...
    try:
        ...
        for item in data:
            ...
            existing_med = db.query(Medication).filter(Medication.name == item_name).first()
            if existing_med:
                # Add ingredients to existing MFDS medication
                for ing in med_ing_objs:
                    if ing not in existing_med.ingredients_list:
                        existing_med.ingredients_list.append(ing)
            else:
                # Create new if not present (fallback for tests)
