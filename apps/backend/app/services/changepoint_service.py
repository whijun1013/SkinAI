...
from app.services.medgemma_service import signal_value_to_score

logger = logging.getLogger(__name__)

# 피부 신호 CUSUM 파라미터: 0~3 등급 차이와 누적 이탈량
_CUSUM_K = float(os.getenv("CUSUM_K", "0.3"))
_CUSUM_H = float(os.getenv("CUSUM_H", "2.0"))
# 단위가 서로 다른 연속 요인은 z-score 표준화 후 별도 기준을 사용한다.
_FACTOR_CUSUM_K = float(os.getenv("FACTOR_CUSUM_K", "0.5"))
_FACTOR_CUSUM_H = float(os.getenv("FACTOR_CUSUM_H", "4.0"))
    ...
    for signal in SKIN_SIGNALS:
        ...
        for sid, d in zip(skin_log_ids, dates):
            ...
            if doc and doc.get("signals"):
                val = doc["signals"].get(signal)
                score = signal_value_to_score(val)
...
def get_user_changepoint_summary(db: Session, user_id: int) -> dict:
    """
    오늘 감지 결과에서 window_start_date와 factor별 analysis_method를 반환.
    결과 없으면 기본 14일 fallback.
    """
    today = date.today()
    rows = (
