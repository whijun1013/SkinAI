...
async def get_medgemma_task_status(skin_log_id: int, user_id: int) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"[MongoDB] Failed to get MedGemma task status for skin_log_id={skin_log_id}: {exc}")
        return {"status": "failed", "skin_log_id": skin_log_id, "error": "Database error"}
