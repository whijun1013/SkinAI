...
def _build_current_cosmetics(db: Session, user_id: int) -> list[dict]:
    ...
    for row in rows:
        if row.product is None or row.product.product_name is None:
            continue
        ingredient_groups = _build_cosmetic_ingredient_groups(row.product.ingredients_list)
        result.append(
            {
                ...
                "irritant_ingredients": sorted(
                    {
                        ...
                        and (ingredient.is_irritant is True or is_taxonomy_irritant_candidate(ingredient.name))
                    }
                ),
                "high_comedogenic_ingredients": sorted(
                    {
                        ingredient.name
                        for ingredient in row.product.ingredients_list
                        if ingredient.comedogenic is not None
                        and ingredient.comedogenic >= 3
                        and ingredient.name is not None
                    }
                ),
                "functional_ingredients": sorted(
                    {
                        ingredient.name
                        for ingredient in row.product.ingredients_list
                        if ingredient.name is not None
                        and is_functional_cosmetic_ingredient(ingredient.name)
                    }
                ),
...
def _build_cosmetic_ingredient_groups(ingredients) -> list[dict[str, str]]:
    groups = []
    seen = set()
    for ingredient in ingredients:
        if ingredient.name is None:
            continue
        for group in classify_ingredient_name(ingredient.name):
            key = (group["key"], group["ingredient"])
            if key in seen:
                continue
            groups.append(group)
            seen.add(key)
    return sorted(groups, key=lambda item: (item["key"], item["ingredient"]))


def _build_cosmetic_events_by_date(
    db: Session, user_id: int, start_date: date, end_date: date
) -> dict[date, dict]:
    """lookback 기간 내 화장품 시작/종료 이벤트를 날짜별로 그룹한다."""
    rows = (
        db.query(UserCosmetic)
    ...
    events: dict[date, dict] = defaultdict(lambda: {"cosmetic_started": [], "cosmetic_ended": []})
    for row in rows:
        if row.product is None or row.product.product_name is None:
            continue
        product_name = row.product.product_name
        if row.started_at and start_date <= row.started_at <= end_date:
            events[row.started_at]["cosmetic_started"].append(product_name)
        if row.ended_at and start_date <= row.ended_at <= end_date:
            events[row.ended_at]["cosmetic_ended"].append(product_name)
    return dict(events)


def _build_medication_events_by_date(
    db: Session, user_id: int, start_date: date, end_date: date
) -> dict[date, dict]:
    """lookback 기간 내 약물 복용 시작/종료 이벤트를 날짜별로 그룹한다."""
    rows = (
    ...
    events: dict[date, dict] = defaultdict(lambda: {"medication_started": [], "medication_ended": []})
    for row in rows:
        if row.medication is None or row.medication.name is None:
            continue
        med_name = row.medication.name
        if row.started_at and start_date <= row.started_at <= end_date:
            events[row.started_at]["medication_started"].append(med_name)
        if row.ended_at and start_date <= row.ended_at <= end_date:
            events[row.ended_at]["medication_ended"].append(med_name)
    return dict(events)
...
def build_analysis_context(
    ...
    try:
        ...
        cosmetic_events_by_date = _build_cosmetic_events_by_date(
            db, user_id, start_date, end_date
        )
        medication_events_by_date = _build_medication_events_by_date(
            db, user_id, start_date, end_date
        )
        ...
        for logged_at in days:
            ...
            if skin is not None:
                ...
                if medgemma is not None:
                    is_unusable = medgemma.get("photo_quality", "pass") == "fail"
                    if not is_unusable:
                        skin_payload["medgemma"] = medgemma

                    trend_item = dict(medgemma)
                    trend_item["date"] = logged_at.isoformat()
                    trend_item["usable"] = not is_unusable
                    medgemma_trend_input.append(trend_item)
            cosmetic_events = cosmetic_events_by_date.get(logged_at, {})
            medication_events = medication_events_by_date.get(logged_at, {})
            timeline.append(
                {
                    ...
                    "cosmetic_started": cosmetic_events.get("cosmetic_started", []),
                    "cosmetic_ended": cosmetic_events.get("cosmetic_ended", []),
                    "medication_started": medication_events.get("medication_started", []),
                    "medication_ended": medication_events.get("medication_ended", []),
                }
            )
