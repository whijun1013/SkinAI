...
def _create_client(settings: dict[str, str]):
    try:
        ...
        raise SkinTendencyLLMError("OpenAI client is not installed") from exc
    ...
    return [
        {
            ...
            "content": (
                ...
                "the user reacts to. "
                "Write as observational reference, not a medical diagnosis — provide skincare analysis only."
            ),
        },
        {
            "role": "user",
...
def _validate_result(result) -> str:
    if not isinstance(result, dict):
        raise SkinTendencyLLMResponseError("OpenAI response must be a JSON object")
...
def get_skin_tendency(
    ...
    try:
        ...
        raise SkinTendencyLLMError("OpenAI request failed") from exc
