...
def _detect_face_with_mediapipe(img_cv: np.ndarray, model_path: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        logger.error(f"MediaPipe face detection failed during execution: {e}")
        return True, f"얼굴 탐지 실행 중 오류 발생: {str(e)}"
    ...
    try:
        ...
        if model_path:
            ...
            if not face_detected:
                return ValidationResult(is_valid=False, warning=warning_msg)
            if warning_msg:
                # 얼굴 탐지 과정에서 오류가 발생했으나 차단하지는 않는 경우 (is_valid=True)
                return ValidationResult(is_valid=True, warning=warning_msg)
        else:
            logger.info("MEDIAPIPE_FACE_DETECTOR_MODEL is not configured. Skipping face detection check.")
            
        return ValidationResult(is_valid=True)
        
    except Exception as e:
        logger.warning(f"Image quality validation failed: {e}")
        # 품질 평가 도중 에러가 나도 기본 업로드를 막지 않도록 True 처리하되 Warning 기록
        return ValidationResult(is_valid=True, warning=f"품질 검사 오류: {str(e)}")
