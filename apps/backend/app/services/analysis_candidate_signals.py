...
def build_candidate_signals(context: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    signals.extend(_build_cosmetic_signals(current.get("current_cosmetics") or [], timeline))
...
def _build_cosmetic_signals(cosmetics: list[dict[str, Any]], timeline: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ingredient_names = []
    ...
    stopped_days = []
    for day in timeline:
        if day.get("cosmetic_started"):
            started_days.append(day.get("date"))
        if day.get("cosmetic_ended"):
            stopped_days.append(day.get("date"))

    if started_days:
        signals.append(
            _count_signal(
                agent_type="cosmetics",
                factor_type="cosmetic",
                factor_key="cosmetic_new_product",
                label="새 화장품 시작",
                count=len(started_days),
                recent_count=_recent_count(started_days, timeline),
                base=0.45,
                per_day=0.0,
                per_recent_day=0.1,
                max_score=0.85,
    ...
    if stopped_days:
        signals.append(
            _count_signal(
                ...
                label="화장품 사용 중단",
                count=len(stopped_days),
                recent_count=_recent_count(stopped_days, timeline),
                base=0.45,
                per_day=0.0,
                per_recent_day=0.1,
                max_score=0.85,
