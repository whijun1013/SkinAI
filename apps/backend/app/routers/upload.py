...
from app.services.blob_storage import build_blob_url, sign_blob_read_url, upload_blob
...
def upload_to_blob_storage(file_content: bytes, container_name: str, blob_name: str) -> str:
    try:
        return upload_blob(container_name, blob_name, file_content)
    except Exception as e:
        raise Exception(f"로컬 파일 저장 실패: {str(e)}")


def generate_filename(user_id: int, log_type: str, meal_type: Optional[str] = None) -> str:
    """파일명에 user_id 등 민감 정보를 포함하지 않도록 익명화 (UUID 기반)"""
    today = datetime.now().strftime("%Y%m%d")
    unique_id = uuid4().hex[:16]
    
    if log_type == 'skin':
        return f"skin_{today}_{unique_id}.jpg"
    elif log_type == 'diet':
        # meal_type이 영어라도 최소한의 메타데이터로만 활용
        return f"diet_{today}_{meal_type or 'unknown'}_{unique_id}.jpg"
    else:
