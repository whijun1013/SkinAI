...
class FoodItem(Base):
    ...
    caffeine = Column(Numeric(8, 2), nullable=True)
    glycemic_index = Column(Numeric(5, 2), nullable=True)
    serving_size = Column(Numeric(8, 2), nullable=True)
    serving_unit = Column(String(20), nullable=True)
    is_canonical = Column(Boolean, nullable=False, server_default='0')
    canonical_id = Column(BigInteger, ForeignKey("food_item.id", ondelete="SET NULL"), nullable=True, index=True)
    created_at = Column(DateTime, nullable=False, server_default=func.now())
    updated_at = Column(DateTime, nullable=True, onupdate=func.now())

    canonical = relationship("FoodItem", remote_side=[id])
