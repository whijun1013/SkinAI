...
from app.services.medgemma_service import build_medgemma_handoff_payload, extract_json, get_medgemma_prompt
...
class MedGemmaLocalRunner:
    ...
    def analyze_image_url(self, image_url: str, *, max_tokens: int = None, max_image_side: int = None) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
        ...
        if max_tokens is None:
            max_tokens = int(os.getenv("MEDGEMMA_MAX_TOKENS", "600"))
...
async def process_one_task(
    *,
    ...
    try:
        ...
        t_mongo = time.time()
        await update_skin_ai_result_medgemma(
            skin_log_id=skin_log_id,
