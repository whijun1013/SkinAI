...
class TestUserMedications(unittest.TestCase):
    ...
    def test_medications_search_include_inactive(self):
        # Create an inactive medication
        inactive_med = Medication(id=3, name="비활성약", is_active=False)
        self.db.add(inactive_med)
        self.db.commit()

        # By default, inactive is excluded
        resp_default = self.client.get("/medications/search?q=비활성")
        self.assertEqual(resp_default.status_code, 200)
        self.assertEqual(len(resp_default.json()), 0)

        # Explicitly include inactive
        resp_include = self.client.get("/medications/search?q=비활성&include_inactive=true")
        self.assertEqual(resp_include.status_code, 200)
        self.assertEqual(len(resp_include.json()), 1)
        self.assertEqual(resp_include.json()[0]["name"], "비활성약")
