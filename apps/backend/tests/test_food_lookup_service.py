...
def test_food_item_barcode_and_canonical_fields(db_session):
    canonical_item = FoodItem(id=200, name="원조 바나나맛 우유", source="dummy", calories=200, is_canonical=True)
    duplicate_item = FoodItem(id=201, name="바나나맛 우유 (프로모션)", source="dummy", calories=200, barcode="8801056000000", canonical_id=200)
    
    db_session.add_all([canonical_item, duplicate_item])
    db_session.commit()
    
    fetched_dup = db_session.query(FoodItem).filter(FoodItem.barcode == "8801056000000").first()
    assert fetched_dup is not None
    assert fetched_dup.canonical_id == 200
    assert fetched_dup.canonical.name == "원조 바나나맛 우유"
