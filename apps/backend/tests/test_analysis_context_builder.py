...
class TestAnalysisContextBuilder(unittest.TestCase):
    ...
    def test_medgemma_handoff_is_included_without_raw_scores(self):
        ...
        self.assertEqual(medgemma["signals"]["redness"], 1)
    ...
    def test_medgemma_handoff_is_excluded_if_rejected_or_unusable(self):
        ...
        result_rejected = build_analysis_context(
            self.db, self.user_id, trigger.id,
            medgemma_handoffs={
                trigger.id: {
                    "source": "medgemma",
    ...
    def test_diet_environment_coverage_flags_and_external_api_non_use(self):
        ...
        self.assertEqual(
            day["environment"],
            {"temperature": 22.0, "humidity": 40.0, "pm10": None, "pm25": 30.0, "uv": 3.0},
        )
        self.assertEqual(
    ...
    def test_current_cosmetics_medications_summary_and_data_quality(self):
        ...
        comedogenic = CosmeticIngredient(name="Isopropyl Myristate", comedogenic=4)
        functional = CosmeticIngredient(name="Retinol", is_irritant=False)
        safe = CosmeticIngredient(name="Water", is_irritant=False)
        current_product = CosmeticProduct(brand="Brand", product_name="Current")
        current_product.ingredients_list = [safe, irritant, comedogenic, functional]
        ...
        self.assertEqual(cosmetics[0]["high_comedogenic_ingredients"], ["Isopropyl Myristate"])
        self.assertEqual(cosmetics[0]["functional_ingredients"], ["AHA", "Retinol"])
        self.assertEqual(
            cosmetics[0]["ingredient_groups"],
            [
                {"key": "exfoliating_acid", "label": "Exfoliating acid", "ingredient": "AHA"},
        ...
        self.assertEqual(
            ...
            [("aha", "AHA"), ("relevant", "Relevant"), ("cosmetic_new_product", "새 화장품 시작")],
        )
    ...
    def test_context_includes_visual_trends_and_primary_context(self):
        ...
        handoffs = {
            past_log.id: {
                ...
                "usable": True,
                "confidence": "high",
                "signals": {"barrier": 0, "active_lesion": 0, "redness": 0}
            },
            trigger.id: {
                "source": "medgemma",
                "recommendation": "use",
                "usable": True,
                "confidence": "high",
                "signals": {"barrier": 3, "active_lesion": 0, "redness": 0},
        ...
        result_valid = build_analysis_context(
            self.db, self.user_id, trigger.id, medgemma_handoffs=handoffs
        )
        self.assertIn("visual_observation_trends", result_valid)
        trends = result_valid["visual_observation_trends"]
        self.assertEqual(trends["worsened_signals"], ["barrier"])
        self.assertIn("barrier", trends["score_drop_overlap_signals"])
    ...
    def test_medication_events_in_timeline(self):
        trigger = self._skin_log(self.end_date)
        medication = Medication(name="Test Med")
        self.db.add(medication)
        self.db.flush()
        
        # UserMedication started and ended in the lookback period
        start_dt = self.end_date - timedelta(days=5) # 2026-05-27
        end_dt = self.end_date - timedelta(days=2)   # 2026-05-30
        ...
        day_start = next(d for d in result["daily_timeline"] if d["date"] == start_dt.isoformat())
        self.assertEqual(day_start["medication_started"], ["Test Med"])
        
        day_end = next(d for d in result["daily_timeline"] if d["date"] == end_dt.isoformat())
        self.assertEqual(day_end["medication_ended"], ["Test Med"])
