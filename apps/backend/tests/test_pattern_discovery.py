...
from app.services.medgemma_service import SIGNAL_LEVELS
    ...
    *,
    ...
    return {
        ...
        "skin": {
            ...
            "medgemma": {
                "signals": {
                    "active_lesion": SIGNAL_LEVELS[active_lesion],
                    "redness": SIGNAL_LEVELS[redness],
                    "barrier": SIGNAL_LEVELS[barrier],
                },
            },
        },
        "diet": [{"meal": "점심", "foods": foods or []}] if foods else [],
...
def _food(*, tags=None, flags=None):
    return {
        "name": "test food",
        "skin_tags": tags or [],
        "flags": flags or [],
...
class TestPatternDiscovery(unittest.TestCase):
    ...
    def test_uses_medgemma_signals_when_available(self):
        ...
        self.assertEqual(high_sugar["affected_signal_label"], "트러블/뾰루지")
    ...
    def test_uses_barrier_signal(self):
        """새 3신호 계약의 barrier 값을 직접 사용한다."""
        start = date(2026, 5, 19)
        exposure_offsets = {0, 4, 8}
        timeline = []
        for offset in range(14):
