...
class TestAnalysisWindow(unittest.TestCase):
    ...
    def test_recent_changepoint_sets_dynamic_window(self):
        target = date(2026, 6, 14)
        self.assertEqual(
            _resolve_analysis_window_start(
                target,
    ...
    def test_normalizes_datetime_target_before_comparing_date_candidate(self):
        self.assertEqual(
            _resolve_analysis_window_start(
    ...
    def test_request_eligibility_uses_base_window_not_dynamic_window(
        self,
        mock_get_skin_log,
        mock_no_duplicate,
        mock_changepoint,
        ...
        request = create_analysis_request(db, user_id=1, skin_log_id=10)
