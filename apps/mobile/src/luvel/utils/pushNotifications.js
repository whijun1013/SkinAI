...
const DEVICE_ID_KEY = "luvel_push_device_id";
...
async function getOrCreateDeviceId() {
  ...
  const deviceId = `luvel-${Platform.OS}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
...
export async function registerDevicePushToken() {
  try {
    ...
    devLog("[Notifications] push token registered");
    return { success: true };
  } catch (error) {
