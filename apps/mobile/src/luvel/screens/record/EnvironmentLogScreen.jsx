...
const LEVEL_COLORS = {
  good: { bg: '#E8EEDD', text: RECORD_COLORS.olive, border: 'rgba(79, 96, 60, 0.2)' },
  ...
  return (
    <View style={styles.metricTile}>
      <View style={styles.metricIcon}>
        <Ionicons name={icon} size={16} color={RECORD_COLORS.olive} />
      </View>
    ...
    <SubScreenRoot onBack={onBack}>
      ...
      >
        {error && !isInitialLoad ? (
          ...
          <StatusBanner
            icon={sortedLogs.length > 0 ? 'cloud-outline' : 'cloud-offline-outline'}
            text={statusText}
            variant={sortedLogs.length > 0 ? 'info' : 'empty'}
          />
        )}
              ...
              {buildEnvironmentMetrics(latestLog)
                ...
                .map((metric) => (
                  <View key={metric.key} style={styles.summaryItem}>
                    <Ionicons name={metric.icon} size={15} color={RECORD_COLORS.olive} />
                    <Text style={styles.summaryValue}>{metric.value}</Text>
                    <Text style={styles.summaryLabel}>{metric.label}</Text>
                  </View>
                ))}
            </View>
            {summaryText ? <Text style={styles.summaryCaption}>{summaryText}</Text> : null}
          </SectionCard>
        ) : null}

        {sortedLogs.length === 0 && !isInitialLoad ? (
          <SectionCard title="아직 환경 기록이 없어요" subtitle={emptyGuide}>
            <View style={styles.emptyBox}>
              <View style={styles.emptyIcon}>
                <Ionicons name="cloud-offline-outline" size={30} color={RECORD_COLORS.olive} />
              </View>
              <Text style={styles.emptyTitle}>
                {isToday ? '식단 기록 후 자동으로 채워져요' : '이 날 환경 데이터가 없어요'}
              </Text>
              <Text style={styles.emptyDesc}>
                식단 사진에 위치 정보가 있거나, 저장 시 지역명을 입력하면 주변 기온·습도·미세먼지를
                함께 모아둡니다.
              </Text>
            </View>
          </SectionCard>
...
const styles = StyleSheet.create({
  ...
  retryBtnText: { fontSize: 13, fontWeight: '800', color: RECORD_COLORS.olive },
});
