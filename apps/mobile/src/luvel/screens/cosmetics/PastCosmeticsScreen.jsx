      ...
      <SubScreenRoot onBack={onBack} enabled={!isDetailOpen && !isDatePickerOpen}>
        ...
        >
          ...
          {showInitialLoading ? (
            <>
              <SectionHeader />
              <SectionCard title="기록 불러오는 중">
                <View style={styles.loadingPanel}>
                  <ActivityIndicator size="small" color={RECORD_COLORS.olive} />
                  <Text style={styles.loadingText}>이전 사용 기록 불러오는 중...</Text>
                </View>
              </SectionCard>
            </>
          ) : showLoadError ? (
            ...
            <SectionCard title="종료 기록 없음">
              <SearchEmptyBox
                icon="time-outline"
                title="이전에 사용한 제품이 없어요"
                description="사용을 종료한 제품이 여기에 기록돼요"
              />
            </SectionCard>
          ) : (
            <>
