                  ...
                  <View style={styles.detailGrid}>
                    {detail.manufacturer && (
                      <View style={styles.detailRow}>
                        ...
                        <Text style={styles.detailValue}>{detail.manufacturer}</Text>
                      </View>
                    )}
                    {detail.valid_term && (
                      <View style={styles.detailRow}>
                        <Text style={styles.detailLabel}>유효기간</Text>
                        <Text style={styles.detailValue}>{detail.valid_term}</Text>
                      </View>
                    )}
                    {detail.storage_method && (
                      <View style={styles.detailRow}>
                        <Text style={styles.detailLabel}>보관방법</Text>
                        <Text style={styles.detailValue}>{detail.storage_method}</Text>
                      </View>
                    )}
                    {detail.material_name && (
                      <View style={[styles.detailRow, styles.detailRowVertical]}>
                        <Text style={styles.detailLabel}>원료성분</Text>
                        <Text style={styles.detailValueMultiline}>{detail.material_name}</Text>
                      </View>
                    )}
                  </View>
                  {detail.form ? (
...
const styles = StyleSheet.create({
  ...
  detailRowVertical: { flexDirection: 'column', gap: 4 },
  detailLabel: { width: 60, fontSize: 12, fontWeight: '700', color: RECORD_COLORS.muted, marginTop: 2 },
  detailValue: { flex: 1, fontSize: 13, fontWeight: '600', color: RECORD_COLORS.text, lineHeight: 18 },
  detailValueMultiline: { fontSize: 13, fontWeight: '500', color: RECORD_COLORS.text, lineHeight: 20 },
