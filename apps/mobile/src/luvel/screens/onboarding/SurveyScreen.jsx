...
import SkinLogEntry from "../record/SkinLogEntry";
...
export default function SurveyScreen({ onLogout }) {
  ...
  const [isSkinLogVisible, setIsSkinLogVisible] = useState(false);
  ...
  const handleStepThreeNext = () => {
    if (medications.length === 0) {
      Alert.alert(
        "약물 정보 없이 넘어갈까요?",
        "복용 중인 약물이 없다면 그대로 넘어가도 됩니다. 나중에 추가할 수 있습니다.",
        [
          { text: "계속 입력하기", style: "cancel" },
          { text: "다음", onPress: () => goNext() },
        ]
      );
      return;
    }
        ...
        <View style={styles.screen} {...panResponder.panHandlers} collapsable={false}>
          ...
          >
            ...
            {currentStep === 3 && (
              loadError && !isInitialLoading ? (
                ...
                <StepThree
                  ...
                  onSkipMedications={handleStepThreeNext}
                  onPrevious={goPrevious}
                  onSave={handleStepThreeNext}
                />
              )
            )}
            ...
            {currentStep === 4 && (
              <StepFour
                isSaving={isSavingSurvey}
                onOpenSkinLog={() => setIsSkinLogVisible(true)}
                onSkipSkinLog={saveSurvey}
                onPrevious={goPrevious}
              />
            )}
          ...
          <Modal
            visible={isSkinLogVisible}
            animationType="slide"
            presentationStyle="fullScreen"
            onRequestClose={() => setIsSkinLogVisible(false)}
          >
            <SkinLogEntry
              onBack={() => setIsSkinLogVisible(false)}
              onDataChanged={() => {
                setIsSkinLogVisible(false);
                saveSurvey();
              }}
            />
          </Modal>
