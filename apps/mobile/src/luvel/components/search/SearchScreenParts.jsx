...
import { formatKoreanDate, getTodayString, isTodayDate } from './searchDateUtils';
  ...
  return (
    <View style={searchStyles.searchWrap}>
      <View style={searchStyles.searchIconCircle}>
        <Ionicons name="search-outline" size={18} color={RECORD_COLORS.olive} />
      </View>
      <TextInput
...
export function CategoryChipRow({ categories, selected, onSelect }) {
  return (
    <View style={searchStyles.categoryRow}>
      {categories.map((category) => {
        ...
        return (
          <TouchableOpacity
            key={category}
            style={[searchStyles.categoryChip, active && searchStyles.categoryChipActive]}
            onPress={() => onSelect(category)}
            activeOpacity={0.78}
          >
            <Text
              style={[searchStyles.categoryChipText, active && searchStyles.categoryChipTextActive]}
            >
              {category}
            </Text>
          </TouchableOpacity>
...
export function SearchResultsSection({ loading, count, isEmpty, emptyText, children }) {
...
export function RegisterDateSection({ value, onChange, editable = true, hint, onPressSelectDate }) {
  ...
  const todaySelected = isTodayDate(value);

  return (
    <>
      <Text style={searchStyles.panelLabel}>사용 시작일</Text>

      <View style={searchStyles.dateQuickRow}>
        <TouchableOpacity
          style={[searchStyles.dateQuickChip, todaySelected && searchStyles.dateQuickChipActive]}
          onPress={() => onChange(getTodayString())}
          disabled={!editable}
          activeOpacity={0.78}
        >
          <Text
            style={[
              searchStyles.dateQuickChipText,
              todaySelected && searchStyles.dateQuickChipTextActive,
            ]}
          >
            오늘
          </Text>
