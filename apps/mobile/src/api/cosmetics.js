...
export async function requestRegistration(brandOrPayload, productName) {
  const payload =
    typeof brandOrPayload === 'object' && brandOrPayload !== null
      ? brandOrPayload
      : { brand: brandOrPayload, product_name: productName };
  const response = await apiClient.post('/cosmetics/pending', payload);
  return response.data;
}
