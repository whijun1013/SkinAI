...
/**
 * 클라우드 스토리지에 피부 사진 업로드 (기본: DB 저장 없음).
 *
...
export async function uploadSkinPhoto(imageUri, { createLog = false } = {}) {
  ...
  devLog('[Skin] ① Blob 업로드 시작', { userId, createLog });
  ...
  devLog('[Skin] ① Blob 업로드 완료', { imageUrl: response.data?.imageUrl });
...
export async function updateSkinLog(logId, payload) {
  devLog('[Skin] ② DB 수정 요청', { logId, overall_score: payload.overall_score });
