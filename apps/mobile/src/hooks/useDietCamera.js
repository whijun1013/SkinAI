...
async function getCurrentLocation() {
  try {
    ...
    devLog("[Diet] 위치 조회 실패:", e.message);
    return { lat: null, lng: null };
  }
}
...
function createDietCaptureHandlers({
  ...
  const tryBackgroundLocation = async (payload) => {
    ...
    try {
      ...
      devLog("[Diet] 백그라운드 위치 처리 실패:", e.message);
    }
  };
      ...
      try {
        ...
        if (perms.status === "granted") {
          ...
          if (last) {
            ...
            devLog("[Diet] 위치 확정 (캐시)", { captured_lat: parsedLat, captured_lng: parsedLng });
          }
        }
      } catch (_) {}
    }
  ...
  useEffect(() => {
    const sub = AppState.addEventListener("change", async (nextState) => {
      ...
      try {
        ...
        devLog("[Diet] 앱 복귀 후 위치 반영 완료", { lat, lng });
      } catch (e) {
        devLog("[Diet] 앱 복귀 위치 재시도 실패:", e.message);
      }
    });
    return () => sub.remove();
  }, []);
