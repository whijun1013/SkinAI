...
const runSocialAuthSession = ({ authUrl, provider, set, logLabel = '소셜 로그인' }) =>
  new Promise((resolve) => {
    ...
    const subscription = Linking.addEventListener('url', async ({ url }) => {
      ...
      try {
        devLog(`${logLabel} 콜백 url:`, url);
        const params = parseCallbackParams(url);
        devLog(`${logLabel} 콜백 params:`, params);
...
const useAuthStore = create((set) => ({
  ...
  register: async (payloadOrEmail, password, name) => {
    ...
    try {
      ...
      devLog('회원가입 에러:', error.response?.data);
    ...
    try {
      ...
      devLog('회원가입 후 자동 로그인 실패:', error.message);
  ...
  socialLogin: async (provider) => {
    ...
    devLog('소셜 로그인 redirectUri:', redirectUri);
    devLog('소셜 로그인 authUrl:', authUrl);
  ...
  appleLogin: async (identityToken, fullName) => {
    set({ isLoading: true, error: null });
    try {
      const data = await authAPI.appleLogin(identityToken, fullName);
      const params = {
      ...
      return await finalizeSocialCallback(params, 'apple', set);
    } catch (error) {
      console.error('애플 로그인 에러:', error.response?.data || error.message);
      const message = error.response?.data?.detail || '애플 로그인에 실패했습니다';
      set({ error: message, isLoading: false });
      return { success: false, error: message };
    }
  },
  ...
  completeOnboardingProfile: async (payload) => {
    ...
    try {
      ...
      devLog('온보딩 프로필 저장 에러:', error.response?.data || error.message);
